...
def plot_occupation(raw_data, restaurant, save_path=None):
    ...
    for i, (r_name, res_sub, cus_sub, total_seats,
        ...
        served = cus_sub.dropna(subset=["start_service_time", "leave_time"])
        event_times = set()
        for _, row in served.iterrows():
            event_times.add(int(row["start_service_time"]))
            event_times.add(int(row["leave_time"]))

        for t in sorted(event_times):
            if t in occ_series.index:
                rate_at_t = occ_series[t] * 100
                ax.axvline(t, color="grey", linewidth=0.6,
                           linestyle=":", alpha=0.6)
                ax.annotate(f"t={t}",
                            xy=(t, rate_at_t),
...
if __name__ == "__main__":
    ...
    plot_occupation(raw_data, restaurant,
                    save_path="/mnt/user-data/outputs/occupation_rate.png")
